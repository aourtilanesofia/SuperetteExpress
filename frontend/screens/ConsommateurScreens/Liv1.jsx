...
const Liv1 = ({ route }) => {
  ...
  const notifyLivreur = async (livreurId) => {
    try {
      const response = await fetch(`http://192.168.43.145:8080/api/v1/livreur/assigner`, {
  ...
  useEffect(() => {
    const checkCommandeLivraison = async () => {
      try {
        const response = await fetch(`http://192.168.43.145:8080/api/commandes/${numeroCommande}/livraison`);
  ...
  const assignerCommandeAuLivreur = async (numeroCommande, livreurId) => {
    try {
      const response = await fetch(`http://192.168.43.145:8080/api/commandes/assigner`, {
  ...
  const fetchLivreur = async () => {
    try {
      ...
      const livreurResponse = await fetch(
        `http://192.168.43.145:8080/api/v1/livreur/nearby?longitude=${destination.longitude}&latitude=${destination.latitude}&maxDistance=10000`
  ...
  const annulerCommande = async () => {
    try {
      const response = await fetch(
        `http://192.168.43.145:8080/api/commandes/cancel/${commande.numeroCommande}`,

...
const AcceuilCommerçant = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchCategorieCount = async () => {
      try {
        const response = await fetch('http://192.168.1.38:8080/api/categories/count');
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchProductsCount = async () => {
      try {
        const response = await fetch('http://192.168.1.38:8080/api/produits/count');
  ...
  useEffect(() => {
    const fetchTodayOrdersCount = async () => {
      try {
        const response = await fetch('http://192.168.1.38:8080/api/commandes/count/today');

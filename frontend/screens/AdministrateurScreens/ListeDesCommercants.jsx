...
const ListeDesCommercants = () => {
  ...
  const fetchCommercants = async () => {
    try {
      const response = await fetch("http://192.168.43.145:8080/api/v1/commercant/tousCommercants");
  ...
  const deleteCommercant = async (id) => {
    Alert.alert("Confirmation", "Voulez-vous vraiment supprimer cet utilisateur ?", [
      { text: "Annuler", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await fetch(`http://192.168.43.145:8080/api/v1/commercant/supCommercant/${id}`, { method: "DELETE" });
  ...
  const toggleStatus = async (id) => {
    try {
      const response = await fetch(`http://192.168.43.145:8080/api/v1/commercant/status/${id}`, {

...
const ListeDesLivreurs = () => { 
  ...
  const fetchLivreurs = async () => {
    try {
      const response = await fetch("http://192.168.1.38:8080/api/v1/livreur/tousLivreurs");
  ...
  const deleteLivreur = async (id) => {
    Alert.alert("Confirmation", "Voulez-vous supprimer cet utilisateur ?", [
      {text: "Annuler", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await fetch(`http://192.168.1.38:8080/api/v1/livreur/refuser/${id}`, { method: "DELETE" });
  ...
  const validerLivreur = async (id) => {
    ...
    try {
      const response = await fetch(`http://192.168.1.38:8080/api/v1/livreur/valider/${id}`, {
        method: "PUT",

...
const ListeDesClients = () => {
  ...
  const fetchClients = async () => {
    try {
      const response = await fetch("http://192.168.43.145:8080/api/v1/consommateur/tousConsommateur");
  ...
  const deleteClient = async (id) => {
    Alert.alert("Confirmation", "Voulez-vous vraiment supprimer cet utilisateur ?", [
      { text: "Annuler", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await fetch(`http://192.168.43.145:8080/api/v1/consommateur/supConsommateur/${id}`, { method: "DELETE" });
  ...
  const toggleStatus = async (id) => {
    try {
      const response = await fetch(`http://192.168.43.145:8080/api/v1/consommateur/status/${id}`, {
